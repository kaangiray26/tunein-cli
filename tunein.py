...
def screen(col,row):
    os.system("printf '\e[8;%s;%st'" %(row,col))
...
def scrape(url,keyword):
    ...
    for i in type.keys():
        ...
        a+=1
    screen(120,len(out)+5)
    ask=[inquirer.List('opt',message="Choose:",choices=lim(out))]
...
def main():
    ...
    if ans1[1:2] == "1":
        ...
        os.system("clear && printf '\e[3J'")
        screen(120,len(favlist.keys())+5)
        ask2=[inquirer.List('opt',message="Choose:",choices=lim(dup2favlist))]
